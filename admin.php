...
function vf_add_vanilla_menu() {
  add_menu_page('XXLBoard', 'XXLBoard', 'manage_options', 'vf-admin-handle', 'vf_admin_page', plugins_url('assets/transparent.png', __FILE__));
  ...
  if (vf_get_option('url', '') != '') {
	 add_submenu_page('vf-admin-handle', 'Forum integrieren', 'Forum integrieren', 'manage_options', 'vf-embed-handle', 'vf_embed_admin_page');
	 add_submenu_page('vf-admin-handle', 'Kommentare integrieren', 'Kommentare integrieren', 'manage_options', 'vf-comment-handle', 'vf_comment_admin_page');
...
<div class="wrap">
   ...
   <h2><?php _e('XXLBoard Setup'); ?></h2>
   <p>Benutze diese Seite, um dein Forum bei XXLBoard mit WordPress zu verknüpfen.</p>
	<?php if ($url == '') { ?>
	 <div class="GetVanilla">
		<h2>Hast du noch kein Forum?</h2>
		<a href="http://xxlboard.de/?comments=1" target="_blank"><span>Erstelle dir dein kostenloses Forum in wenigen Minuten!</span></a>
	 </div>
	<?php
	}
	vf_open_form('url-form');
	?>
		<strong>Gebe die vollständige URL deines Forums ein:</strong>
		<input name="<?php echo vf_get_option_name('url'); ?>" value="<?php echo $url; ?>" class="InputBox" />
		<span class="Progress"></span>
		<span class="Validated"></span>
		<span class="Invalid"></span>
		<em>Bsp. http://beispiel.xxlboard.de/</em>
      <p class="submit">
		  <input type="submit" class="Save" name="save" value="<?php _e('Speichern'); ?>" />
		</p>
   </form>
</div>
<?php
