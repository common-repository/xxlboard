<?php
/**
 * Embed XXLBoard Functions
 */

/**
 * Embed XXLBoard administration page.
 */
...
<div class="wrap">
   ...
   <h2><?php _e('Forum integrieren'); ?></h2>
   <p>Benutzer diese Seite, um dein Forum in einer Seite anzeigen zu lassen.</p>
	<?php vf_open_form('embed-form'); ?>
		<strong>Seite in WordPress</strong>
		<em>Definiere, auf welcher Seite das Forum angezeigt werden soll.</em>
		<div id="edit-slug-box"><?php echo get_sample_permalink_html($post_id); ?></div>
		<?php wp_nonce_field( 'samplepermalink', 'samplepermalinknonce', false ); ?>
		<em>Du kannst die Seite <a href="./post.php?post=<?php echo $post_id; ?>&action=edit">hier</a> anpassen.</em>
		
		<strong>Widget Links</strong>
		<p>
			<label>
				<input type="checkbox" name="<?php echo vf_get_option_name('embed-widgets'); ?>" value="1"<?php echo $embed_widgets == '1' ? ' checked="checked"' : ''; ?> />
				Die Links in den <a href="./admin.php?page=vf-widgets-handle">Widgets</a> sollen auf die o.g. Seite verweisen und nicht auf das Forum (<?php echo $vanilla_url; ?>).
			</label>
		</p>
		...
		<em>Du kannst den Embed Code hier anpassen (optional). <a class="reset-embed-code" href="#">Reset Embed Code</a>.</em>
...
<div class="wrap">
   <div id="icon-options-general" class="icon32"><br /></div>
   <h2><?php _e('Kommentare integrieren'); ?></h2>
	<?php vf_open_form('embed-comments-form'); ?>
	 <p>
		<label>
		  ...
		  Ersetze dein Kommentarsystem durch das XXLBoard-Kommentarsystem. Die Kommentare landen dann in deinem Forum!
		</label>
	 </p>
	 <p>
		<label>
		  <select name="<?php echo vf_get_option_name('embed-categoryid'); ?>"><?php echo $select_options; ?></select>
		  <?php echo __('Kommentare in diese Kategorie einfügen.'); ?>
		</label>
	 </p>
    <p>
       <label>
          ...
          <?php echo __('Versuche, die Kategorie von WordPress im Forum zu übernehmen.'); ?>
       </label>
    </p>
    <p class="submit"><input type="submit" name="save" value="<?php _e('Save Changes'); ?>" /></p>
  </form>
