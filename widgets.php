...
<div class="wrap">
   ...
   <p>Du kannst sehr interessante Widgets zu deinem Blog hinzufügen:</p>
	<ul>
		<li><strong>XXLBoard Disskussion</strong> zeigt die letzten Diskussionen an.</li>
		<li><strong>XXLBoard Aktivitäten</strong> zeigt die letzten Aktivitäten an (Benutzer hat sich registriert etc.).</li>
		<li><strong>XXLBoard Benutzer</strong> zeigt einige Nutzer deines Forums an.</li>
	</ul>
	<p>Die Widgets findest du <a href="./widgets.php">hier</a>.</p>
</div>
	<?php
}
...
//         var_dump($Discussion);
	...
	function vf_widget_discussions_control() {
		...
		$options = get_option(VF_OPTIONS_NAME);
		$title = vf_get_value('widget-discussions-title', $options, 'Aktuelle Diskussionen');
		$categoryid = (int)vf_get_value('widget-discussions-categoryid', $options);
	...
	wp_register_sidebar_widget('vf-widget-discussions', 'XXLBoard Disskussionen', 'vf_widget_discussions', array('description' => 'Zuletzt diskutierte Themen in deinem Forum.'));
...
// Recent Activity Widget
	...
	function vf_widget_activities_control() {
		// Get our options and see if we're handling a form submission.
		$options = get_option(VF_OPTIONS_NAME);
		$title = vf_get_value('widget-activities-title', $options, 'Aktivitäten');
		// $roleid = (int)vf_get_value('widget-activities-roleid', $options);
	...
	// This registers our widget so it appears with the other available
	// widgets and can be dragged and dropped into any active sidebars.
	wp_register_sidebar_widget('vf-widget-activities', 'XXLBoard Aktivitäten', 'vf_widget_activities', array('description' => 'Aktuelle Aktivitäten in deinem Forum (Bsp.: neue Benutzer).'));
...
// Recently Active Users Widget
	...
	function vf_widget_users_control() {
		// Get our options and see if we're handling a form submission.
		$options = get_option(VF_OPTIONS_NAME);
		$title = vf_get_value('widget-users-title', $options, 'Aktive Benutzer');
	...
	// This registers our widget so it appears with the other available
	// widgets and can be dragged and dropped into any active sidebars.
	wp_register_sidebar_widget('vf-widget-users', 'XXLBoard Benutzer', 'vf_widget_users', array('description' => 'Symbole der letzten Benutzer.'));
